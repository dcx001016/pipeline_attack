...
from pipeline_parallel.dist_pp_utils import get_opt_pp_module_virtual
...
def main():
    ...
    parser.add_argument('--model-name', type=str, default='checkpoints/facebook/opt-350m', metavar='S',
                        help='model name or path')
    parser.add_argument('--tokenizer-name', type=str, default='checkpoints/facebook/opt-350m', metavar='S',
    ...
    if args.wandb and get_pipeline_parallel_rank() == args.pipeline_group_size-1:
        wandb.init(project=f"dist_opt_runner-same_magnitude-defense-{args.optimizer}-{args.task_name}-vgpus-{args.pipeline_virtual_gpus}", 
    ...
    pipe = get_opt_pp_module_virtual(args, config, device)
